...
async function main() {
    try {
        ...
        const byNama = await db.collection('pengguna').findOne({ nama: 'Afiif' });

        // Mencari satu dokumen dalam koleksi 'pengguna' berdasarkan ID objek tertentu.
        const byObjectID = await db.collection('pengguna').findOne({ _id: new ObjectId("69327cc5b3acf266b66effd9") });
